...
def warmstart_model(project, Gamma, warm_start, time_limit):

    print('\n\n\ninstance: {}; Gamma = {}\nBasic model w/ warm-start\n------------------------------------------\n'.format(project.name,Gamma))

    model = Model("{}_{}_warmstart_model".format(project.name, Gamma))
    ...
    model.addConstrs(quicksum(f[i,j,k] for j in V if j != 0) == r[i][k] for i in V if i != n+1 for k in K) #flow out of node i

    model.optimize()
    model.write("warmstart_model.sol")
...
sol = warmstart_model(project, Gamma, warm_start, time_limit)
