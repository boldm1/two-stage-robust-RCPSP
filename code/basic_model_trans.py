...
def basic_model_trans(project, Gamma, time_limit): 

    print('\n\n\ninstance: {}; Gamma = {}\nBasic model w/ transitivity constraints\n------------------------------------------\n'.format(project.name,Gamma))

    model = Model("{}_{}_basic_model_trans".format(project.name, Gamma))
    ...
    model.addConstrs(y[i,j] + y[j,i] <= 1 for i in V for j in V if (i != n+1and j != n+1))
    model.addConstrs(y[i,j] >= y[i,p] + y[p,j] - 1 for i in V for j in V for p in V if i != j if i != p if j != p)

    model.optimize()
    model.write("basic_model_trans.sol")
...
Gamma = 3
time_limit = 20*60
sol = basic_model_trans(project, Gamma, time_limit)
